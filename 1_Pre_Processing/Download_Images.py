...
address_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "Locations", "SF.csv"
)
test_address_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "Locations", "Test_SF.csv"
)
FLAGS = None
...
if __name__ == "__main__":
    ...
    if FLAGS.test:
        df_vul = seperate_house_numbers(
            pd.read_csv(test_address_file), column="PROPERTY", city=FLAGS.city
        )
    else:
        df_vul = seperate_house_numbers(
            pd.read_csv(address_file), column="PROPERTY", city=FLAGS.city
        )
    df_vul = tidy_split(df_vul, "NUMBER", sep="+")
    df_vul = ignore_non_int(df_vul)
    df_vul.dropna(axis=0, subset=["TIER"], inplace=True)
    df_vul_clusters = find_similar_clusters(df_vul, column_name="STATUS", cutoff=0.97)

    ## Download buildings by category
    if not FLAGS.G_API_key:
        print(
            "No Google API key provided. Downloading images will only be simulated.",
            "\n",
        )
    ...
    for cluster in df_vul_clusters:
        ...
        results_df = download_images(
            df_vul[df_vul["STATUS"].isin(cluster)],
            results_df,
            dir_path=result_folder,
            class_name="vulnerable",
            category=cluster[0],
            api_key=FLAGS.G_API_key,
            save_tier=False,
        )
