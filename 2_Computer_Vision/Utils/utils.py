...
"""
    ...
    print(
        "Loaded {} feature extractor in {:.2f}sec".format(
            model_name,
            end -
            start))
...
def chunks(l, n, preprocessing_function=None):
    ...
    func = (
        lambda x: x) if (
        preprocessing_function is None) else preprocessing_function

    # in predict_generator, steps argument sets how many times looped through
    # "while True"
    while True:
...
def contents_of_bbox(img, bbox_list, expand=1.0):
    ...
    for i, (xmin, ymin, xmax, ymax, *_) in enumerate(bbox_list):
        ...
        xmax, ymax = int(np.round(xmax // expand)
    ...
    for box_list, label, color in zip(box_list_list, label_list, color_list):
        ...
        for box in box_list:
            ...
            for i in range(thickness):
                draw.rectangle([xmin + i, ymin + i, xmax -
                                i, ymax - i], outline=color)
            draw.rectangle([tuple(text_origin), tuple(
                text_origin + label_size)], fill=color)
