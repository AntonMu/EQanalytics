...
def _main(args):
    ...
    assert config_path.endswith(
        ".cfg"), "{} is not a .cfg file".format(config_path)
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith("convolutional"):
            ...
            conv_bias = np.ndarray(
                shape=(
                    filters,), dtype="float32", buffer=weights_file.read(
                    filters * 4))
            count += filters
            ...
            conv_weights = (
                [conv_weights] if batch_normalize else [
            ...
            all_layers.append(
                MaxPooling2D(
                    pool_size=(
                        size, size), strides=(
                        stride, stride), padding="same")(prev_layer))
