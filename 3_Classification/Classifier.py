...
openings_result_file = os.path.join(
    openings_result_folder,
    "Opening_Results.csv")
openings_classes = os.path.join(
    data_folder, "Model_Weights", "Openings", "data_all_classes.txt"
)
...
if __name__ == "__main__":
    ...
    # Now calculate features such as the width of windows and doors. Also
    # filter everythin that is not a door, window or shop
    opening_df = get_features(
        opening_df, label_dict, label_names=["door", "window", "shop"]
    )

    # Next remove all objects that have more than 75% IoU (and keep the bigger
    # one)
