...
def remove_overlaps(df, iou_threshold=0.75):
    ...
    for image_name in image_names:
        ...
        result_df = result_df.append(
            current_df.iloc[np.unique(np.array(indices_to_keep))]
        )
    return result_df
...
def find_levels(df, threshold=2e-3, save_levels=False):
    ...
    for image_name in image_names:
        ...
        scores = np.array([KMeans(n_clusters=n, random_state=42, n_init=2).fit(
            data).score(data) for n in range(1, min(8, len(data)))]) / (len(y))
        number_of_stories = sum(((np.diff(scores)) > threshold)) + 1
        kmeans = KMeans(
            n_clusters=number_of_stories,
            random_state=42,
            n_init=2).fit(data)
        order_dict = dict(
            zip(
                kmeans.predict(np.sort(kmeans.cluster_centers_.T).T),
                list(range(number_of_stories, 0, -1)),
            )
        )
        current_df.loc[:, "level"] = kmeans.predict(data)
        current_df["level"] = current_df["level"].apply(
...
def draw_levels(
    df,
    target_path,
    colors=["#fdfe02", "#0bff01", "#fe0000", "#fe00f6", "#011efe"],
    suffix="_levels",
):
    ...
    for image_name in image_names:
        ...
        for level in levels:
            for index, row in current_df[current_df["level"]
                                         == level].iterrows():
                draw.rectangle(
                    ((row["xmin"], row["ymin"]), (row["xmax"], row["ymax"])),
                    outline=colors[level % len(colors)],
                    width=2,
                )
                try:
        ...
        source_img.convert("RGB").save(
            os.path.join(
                target_path,
                "".join([os.path.basename(image_name)[:-4], suffix, ".jpg"]),
            ),
            "JPEG",
        )
    return True
...
def calcuate_softness(df, metric="x_len"):
    ...
    for image_name in image_names:
        ...
        if metric == "area":
            result_df = result_df.append(
                pd.DataFrame(
                    [
                        [
                            image_name,
                            sum(current_df[current_df["level"] == 2]["area"])
                            / sum(current_df[current_df["level"] == 1]["area"]),
                        ]
                    ],
                    columns=["image", "score"],
                )
            ...
            scores = []
            for level in [1, 2]:
                # metric[0]+'min' equals xmin for metric = 'x_len' and ymin for
                # metric = 'y_len'
                scores.append(
                    current_df[metric[0] + "_size"].iloc[0]
                    - get_intervall_union(
                        list(
                            zip(
                                current_df[current_df["level"] == level][
                                    metric[0] + "min"
                                ].values,
                                current_df[current_df["level"] == level][
                                    metric[0] + "max"
            ...
            result_df = result_df.append(
                pd.DataFrame(
                    [[image_name, float(scores[0]) / max(float(scores[1]), 1.0)]],
                    columns=["image", "score"],
                )
            )
...
def get_address(df, column="image"):
    df["address"] = df[column].apply(
        lambda x: ((re.findall(r"_\d{1,}?\+.*\+", x))[0][1:]).replace("+", " ")
    )
